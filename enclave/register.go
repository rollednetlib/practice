...
func registrationConfirmation(w http.ResponseWriter, r *http.Request, ps httprouter.Params) {
    fmt.Fprintf(w, "Checking token, %s!\n", ps.ByName("confirmationToken"));
}
...
func main() {
    router := httprouter.New()
 //   router.ServeFiles("/", http.Dir("/"));
    router.GET("/registrationConfirmation/:confirmationToken", registrationConfirmation);
  //  router.GET("/reset", Reset);
  //  router.GET("/signin", Signin);
    router.GET("/register", RegisterGet);
    router.POST("/registerp", RegisterPost);
